...
def plbfgs(
    ...
    print_level: int = 1,  # 0:no print; 1:one line per iter; 2:details.
    ...
    for it in range(max_iter):
        ...
        if it == iters_before_inv_hess0 and inv_hess0:
            print(f"Iter {it+1}: Start using preconditioner")
        ...
        if (
            ...
            print(f"Iter {it+1}: Update preconditioner")
            inv_hess.inv_hess0 = inv_hess0(x)
            inv_hess.hess0 = hess0(x)
        ...
        if print_level >= 1:
            print(
                f"Iter {it+1:>3}: f = {f:.3e}, |g| = {gnorm:.3e}, |g|/|g0| = {gnorm/gnorm0:.3e}"
            )
